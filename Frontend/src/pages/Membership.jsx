            ...
            {plans.map((p, i) => {
              ...
              return (
                <Reveal key={p.slug} delay={i * 0.1}>
                  <motion.div whileHover={{ y: -10 }} className={`relative p-8 rounded-3xl transition-all h-full flex flex-col ${meta.featured ? "bg-luxury text-[color:var(--cream-soft)] shadow-luxe xl:scale-105" : "bg-background border border-border shadow-soft hover:shadow-luxe"}`}>
                    {meta.featured && (
                    ...
                    <div className="mt-7">
                      <div className={`font-display text-5xl font-bold ${meta.featured ? "text-gradient-gold" : "text-gradient-luxury"}`}>{p.price_display || `\u20A6${Number(p.price).toLocaleString()}`}</div>
                      <div className={`text-sm mt-1 ${meta.featured ? "text-[color:var(--cream-soft)]/70" : "text-muted-foreground"}`}>{meta.per}</div>
                    </div>
                    ...
                    <Link to={p.slug === "free" ? "/register" : `/checkout/${p.slug}`} className={`mt-10 block text-center py-4 rounded-full font-bold transition ${meta.featured ? "bg-gold text-[color:var(--emerald-deep)] hover:shadow-glow" : "bg-emerald text-[color:var(--gold-royal)] hover:shadow-glow"}`}>Choose {p.name}</Link>
                  </motion.div>
                </Reveal>
              );
            })}
